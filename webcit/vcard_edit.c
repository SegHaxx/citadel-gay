...
vcField VCStrE[] = {
	...
	{{HKEY("adr")}, StringCluster, VCStr_Addrs, 0, 0,
	 {HKEY("Address:")}},
	{{HKEY("photo")}, Base64BinaryAttachment, NULL, 0, 0,
	 {HKEY("Photo:")}},
	...
	{{HKEY("email;internet")}, EmailAddr, NULL, 0, 0,
	 {HKEY("E-mail:")}},
...
void autoRegisterTokens(long *enumCounter, vcField * vf, StrBuf * BaseStr,
			int layer, long parentCVal)
{
	int i = 0;
...
void PutVcardItem(HashList * thisVC, vcField * thisField,
		  StrBuf * ThisFieldStr, int is_qp, StrBuf * Swap)
{
...
wc_mime_attachment *load_vcard(message_summary * Msg)
{
	...
	while (GetNextHashPos(Msg->AllAttach, it, &len, &Key, &vMime) &&
		...
		if ((strcmp(ChrPtr(Mime->ContentType), "text/x-vcard") == 0) || (strcmp(ChrPtr(Mime->ContentType), "text/vcard") == 0)) {
			VCMime = Mime;
			break;
		}
	}
...
int vcard_GetParamsGetServerCall(SharedMessageStatus * Stat,
				 ...
				 long oper,
				 char *cmd,
				 long len, char *filter, long flen)
{
	vcardview_struct *VS;
...
int vcard_LoadMsgFromServer(SharedMessageStatus * Stat,
			    void **ViewSpecific,
			    message_summary * Msg, int is_new, int i)
{
	wcsession *WCC = WC;
	WCTemplputParams *TP = NULL;
	WCTemplputParams SubTP;
...
void InitModule_VCARD(void) {
	...
	RegisterIterator("MAIL:VCARDS", 0, NULL, CtxGetVcardList, NULL,
			 NULL, CTX_VCARD, CTX_VCARD_LIST, IT_NOFLAG);

	WebcitAddUrlHandler(HKEY("edit_vcard"), "", 0, edit_vcard, 0);
	WebcitAddUrlHandler(HKEY("submit_vcard"), "", 0, submit_vcard, 0);
	WebcitAddUrlHandler(HKEY("vcardphoto"), "", 0,
			    display_vcard_photo_img, NEED_URL);

	RegisterNamespace("VC:ITEM", 2, 2, tmpl_vcard_item,
			  preeval_vcard_item, CTX_VCARD);
	RegisterNamespace("VC:CTXITEM", 1, 1, tmpl_vcard_context_item,
			  NULL, CTX_VCARD_TYPE);
	RegisterNamespace("VC:NAME", 1, 1, tmpl_vcard_name_str,
			  preeval_vcard_name_str, CTX_VCARD);
	RegisterNamespace("VC:MSGNO", 0, 1, tmpl_vcard_msgno, NULL,
			  CTX_VCARD);
	RegisterNamespace("VC:CTXNAME", 1, 1, tmpl_vcard_context_name_str,
			  NULL, CTX_VCARD_TYPE);
	...
	RegisterConditional("VC:HAVE:TYPE", 1, conditional_VC_Havetype,
			    CTX_VCARD);
	RegisterFilteredIterator("VC:TYPE", 1, DefineToToken, NULL, NULL,
				 NULL, filter_VC_ByType, CTX_VCARD_TYPE,
				 CTX_VCARD, IT_NOFLAG);
	RegisterFilteredIterator("VC:TYPE:ITEMS", 0, NULL, getContextVcard,
				 NULL, NULL, filter_VC_ByContextType,
				 CTX_STRBUF, CTX_VCARD_TYPE, IT_NOFLAG);

	RegisterMimeRenderer(HKEY("text/x-vcard"), render_MIME_VCard, 1,
			     201);
	RegisterMimeRenderer(HKEY("text/vcard"), render_MIME_VCard, 1,
			     200);
}
